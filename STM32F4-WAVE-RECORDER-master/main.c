...
uint32_t  counter = 0, oneSecCounter;
uint16_t enableFlag = 0;
				          ...
				          for (i=0; i<16; i++)
				          {
				            ...
				            if (buf_idx< Buffer_Input_SIZE)
				            {
				              ...
				              if (buf_idx1 == Buffer_Input_SIZE)
				              {
				                ...
				                VCP_send_buffer(buffer_input1, Buffer_Input_SIZE * 2);
				            ...
				            else if (buf_idx1< Buffer_Input_SIZE)
				            {
				              ...
				              if (buf_idx == Buffer_Input_SIZE)
				              {
				                buf_idx = 0;
				                /* Send the stored data in the buffer via COM port  */
				                VCP_send_buffer(buffer_input, Buffer_Input_SIZE * 2);
...
static void I2S_Configure(void){
  ...
  SPI_I2S_DeInit(SPI2);
  I2S_InitStructure.I2S_AudioFreq = SAMPLE_FREQUENCY * 2; // Audio is recorded from two channels, so this value must be twice the value in the PDM filter
